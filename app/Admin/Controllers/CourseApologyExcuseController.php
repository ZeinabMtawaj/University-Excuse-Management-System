...
class CourseApologyExcuseController extends AdminController
{
    /**
     ...
     * @var string
     */
    protected $title = 'Course Apology Excuse';
    ...
    {
        $grid = new Grid(new CourseApologyExcuse());
        ...
        $grid->column('file_path', __('admin.file'))->display(function ($file_path) {
            ...
            if ($file_path) {
                ...
                $url = route('file.download', [
                    'model' => 'course-abology-excuse', // This is a unique identifier for the AbsenceExcuse model
                    'folder' => 'course abologies', // The folder where files are stored
    ...
    {
        $show = new Show(CourseApologyExcuse::findOrFail($id));
    ...
    {
        $form = new Form(new CourseApologyExcuse());
...
// Initially, the course dropdown is empty
        ...
        $form->file('file_path', __('admin.file'))
                ->rules('required')
                ->move('course abologies/', date('YmdHis') . '-' . Str::random(7))
                ->disk('private');
